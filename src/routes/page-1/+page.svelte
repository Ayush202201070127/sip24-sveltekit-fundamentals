...
<header class = "bg-white py-4 shadow-md sticky top-0 z-10">
    <div class = "container mx-auto px-4 flex justify-between items-center">
        ...
        <a href="/" class="bg-blue-500 hover:bg-blue-700 text-white font-bold px-4 py-2 rounded">Home</a>
    </div>
</header>
...
<form class="container mx-auto p-5" method="POST" enctype="multipart/form-data">
    ...
    <div class="canvas-container mb-3">
        ...
        {#if files.length > 0}
            <div class="mt-2">
                <button type="button" on:click={() => applyFilter('grayscale')} class="bg-gray-300 hover:bg-blue-900 text-black font-bold py-2 px-4 rounded">Grayscale</button>
                <button type="button" on:click={() => applyFilter('sepia')} class="bg-yellow-200 hover:bg-blue-900 text-black font-bold py-2 px-4 rounded">Sepia</button>
            </div>
        {:else}
            <p class="text-red-500 font-semibold mt-4">Upload the image first.</p>
        {/if}
    </div>
   
    <div class="mb-3">
        <label for="username" class="block mb-2 text-sm font-medium text-gray-900">Username</label>
        <input name="username" id="username" type="text" class="border border-black text-gray-900 text-sm rounded-lg block w-full p-2.5" />
    </div>
    <div class="mb-3">
        <label for="content" class="block mb-2 text-sm font-medium text-gray-900">Description</label>
        <textarea name="content" id="content" class="border border-black text-gray-900 text-sm rounded-lg block w-full p-2.5"></textarea>
    </div>
